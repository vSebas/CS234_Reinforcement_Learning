...
\vspace{-.5in}
\begin{framed}
{\bf Due date: Sunday Feb 1, 2026 at 6:00 PM (18:00) PST}
\\[1em]
...
\\[1em]
Submissions are composed of 3 parts: Assignment 2 Written (PDF file), Assignment 2 LaTeX Written (.zip file), and Assignment 2 Coding (.zip file) 
\end{framed}
...
In this pseudocode:
\begin{itemize}
    \item $D$ is the replay buffer which stores transitions.
...
\begin{enumerate}
    ...
    \item[(a) (3 pts)] (\textbf{written}) What lines in the DQN pseudocode have to be changed to recover the tabular Q-learning algorithm and what would you change them to?

\begin{tcolorbox}[breakable]
%%%%% Start of 1(a) %%%%%
Answer:

The neural network is replaced with a table Q[s,a], and you do not use replay buffers, target networks, or minibatch SGD.

\begin{itemize}
    \item Line 1-2: delete
    \item Line 3: Initialize Q-table Q(s,a) arbitrarily (0 for all s,a)
    \item Line 8: change to \[a_t = \arg \max_a Q(st, a)\] (drop weights)
    \item Line 10-12: delete (no mini-batch loop)
    \item Update subscripts from j to t
    \item Line 16: update to Q-table
        \[r_t + \gamma \max_{a'} Q(s_{t+1}, a')\]
    \item Line 18: replace to
        \[Q(s_t,a_t)\leftarrow Q(s_t,a_t) + \alpha\big(y_t - Q(s_t,a_t)\big)\]
    \item Line 19-20: delete
...
\clearpage 
    \item[(b) (2 pts)] (\textbf{written}) In lecture 2, we introduced a Mars rover problem. What changes to the Mars Rover example may be made such that the tabular Q-learning algorithm will perform extremely poorly? (and then, for example, one might need to use a different algorithm such as DQN)

\begin{tcolorbox}[breakable]
%%%%% Start of 1(b) %%%%%
Answer:

Tabular methods blow up when the state or action spaces are huge or continuous. 

In the Mars Rover example, we could replace the 7 discrete states to something that includes position, velocity, acceleration, orientation, battery level, etc. Then the size of $\mathcal{S}$ becomes massive, and makes the table useless.
The action space could also change to consider continuous wheel torques or steering angles instead of left/right actions. Then $\mathcal{A}$ is continuous and $\max_{⁡a^'} \, Q(s',a')$ over a table becomes impossible

%%%%% End of 1(b) %%%%%
\end{tcolorbox}
\clearpage 

    \item[(c) (3 pts)] (\textbf{written}) Explain why the replay buffer used in DQN is beneficial.
...
\begin{itemize}
    \item Reduces temporal correlations, as samples from training data in an out of order fasion.
    \item Reuses past experience multiple times instead of discarding it after one gradient step.
    \item Stabilize learning as the network is trained on a broader mixture of past experiences rather than the most recent ones.
\end{itemize}
...
\newpage
\section{Policy Gradient Methods (54 pts coding + 21 pts writeup)}
...
\subsection{Coding Questions (54 pts)}
\emph{\textbf{Before starting:} Please read the \texttt{README.md} file for important setup and installation instructions.}

\emph{\textbf{Package version tip:} We recommend creating a conda environment and installing the required packages using \texttt{pip install -r requirements.txt}. If you encounter installation issues, first run \texttt{pip install pip==23.0} to downgrade pip to a version compatible with \texttt{gym==0.21.0}.}
\\\\
...
\emph{\textbf{Note:} The ``batch size" for all the arguments is $\sum T_i$ since we already flattened out all the episode observations, actions, and rewards for you.}
\\
...
\emph{Tip 1: Make sure to check the dimensions of your output.}

\emph{Tip 2: Use \texttt{self(observations)} to invoke the \texttt{forward} call instead of directly referencing the network (so that the shape is correct)}
   
In \texttt{ppo.py}, you need to implement:
\begin{itemize}
\item \texttt{PPO.update\_policy}
\end{itemize}
\emph{Tip 3: When computing the ratios for the PPO update, instead of dividing log-probabilities, try exponentiating the difference of log probabilities.}

\emph{Tip 4: See part (d) for how to run your code.}

   
\subsection{Debugging}

To help debug and verify that your implementation is correct, we provide a set of sanity checks below that pass with a correct implementation. Note that these are not exhaustive (i.e., they do not verify that your implementation is correct) and that you may notice oscillation of the average reward across training. One way is averaging across several seeds.
Across most seeds:
\begin{itemize}
...
\subsection{Writeup Questions (21 pts)}
\begin{enumerate}
...
Given rewards $r_0, \dots, r_{T-1}$ for one trajectory and discount $\gamma\in(0,1)$, compute the returns backwards: 
\[
  G_{T-1}=r_{T-1},\qquad G_t=r_t+\gamma G_{t+1}\quad \text{for } t=T-2,\dots,0
\]

The backwards pass takes $O(T)$ time.
%%%%% End of 2(a) %%%%%
\end{tcolorbox}
\clearpage 
\item[(b) (3 pts)] Consider the cases where the gradient of the clipped PPO loss function equals 0. 
...
The PPO clipped objective at time $t$ is
\[
  L_t^{\mathrm{clip}}(\theta)=\min\Big(z_t(\theta)\hat A_t,\ \mathrm{clip}(z_t(\theta),1-\epsilon,1+\epsilon)\hat A_t\Big)
\]

where
\[
  z_t(\theta)=\frac{\pi_\theta(a_t\mid s_t)}{\pi_{\theta_{\mathrm{old}}}(a_t\mid s_t)}
\]

%Ignoring non-differentiability exactly at the clipping boundary and assuming gradients flow only through the selected branch of the $\min$, 
The gradient w.r.t.\ $\theta$ is zero in:
\begin{enumerate}
  \item If $\hat A_t>0$ and $z_t(\theta)\ge 1+\epsilon$, then the min chooses $(1+\epsilon)\hat A_t$ is selected and is constant in $\theta$, so $\nabla_\theta L_t^{\mathrm{clip}}(\theta)=0$
  \item If $\hat A_t<0$ and $z_t(\theta)\le 1-\epsilon$, then the min chooses $(1-\epsilon)\hat A_t$ is selected and is constant in $\theta$, so $\nabla_\theta L_t^{\mathrm{clip}}(\theta)=0$
  \item If $\hat A_t=0$, then $L_t^{\mathrm{clip}}(\theta)=0$ regardless of $\theta$, so $\nabla_\theta L_t^{\mathrm{clip}}(\theta)=0$
\end{enumerate}

PPO behaves like this as it stops pushing the policy further once the probability ratio moves too far from 1 in the “helpful” direction for that sample
...
REINFORCE uses the on-policy gradient
\[
  \nabla_\theta J(\theta)\approx \sum_{t}\nabla_\theta \log \pi_\theta(a_t\mid s_t)\, G_t
\]
so one can compute $\log\pi_\theta(a_t\mid s_t)$ directly with current $\theta$ at update time. In contrast, PPO explicitly uses the ratio
\[
  z_t(\theta)=\frac{\pi_\theta(a_t\mid s_t)}{\pi_{\theta_{\mathrm{old}}}(a_t\mid s_t)}
\]
During PPO optimization, $\theta$ changes over multiple epochs, but $\pi_{\theta_{\mathrm{old}}}$ must remain fixed as the behavior policy that generated the data. Therefore PPO must store $\log\pi_{\theta_{\mathrm{old}}}(a_t\mid s_t)$  to compute $z_t(\theta)$ during updates.
...
\textbf{\textit{Important:}} Please include the plots (one for each environment) in your write-up, and comment on the performance of each method. \\\\
Additionally, you must \textbf{submit a single \texttt{.zip} file} to \textit{Assignment 2 -- Written Portion LaTeX Source}. This ZIP file must contain your LaTeX source and all generated plot images.

Specifically, ensure that the ZIP file includes the following files \textbf{with the exact naming convention and directory structure}.  
Do \textbf{not} include an enclosing top-level folder; the ZIP file must unpack directly to:
\begin{verbatim}
main.tex
img/
  results-cheetah.png
  results-pendulum.png
  results-cartpole.png
\end{verbatim}


We provide the following performance benchmarks based on typical baseline solutions. Correct implementations may achieve potentially higher rewards than those listed below:
\begin{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{img/results-cartpole.png}
\caption{CartPole results across seeds 1, 2, 3.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{img/results-pendulum.png}
\caption{Pendulum results across seeds 1, 2, 3.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{img/results-cheetah.png}
\caption{HalfCheetah results with seed 1.}
\end{figure}

\textbf{CartPole:} All three methods reach the maximum reward of 200. PPO converges fastest, reaching 200 around iteration 20, and maintains the most stable performance.

\textbf{Pendulum:} All methods reach the maximum reward of 1000. PPO converges fastest and maintains the most stable performance at 1000. Both baseline and no-baseline methods reach 1000 but show more variance and periodic drops.

\textbf{HalfCheetah:} PPO is the only method that maintains stable performance, achieving rewards around 350--430 throughout training. Both vanilla PG methods (baseline and no-baseline) meet the benchmark by reaching rewards above 200 at their peaks (baseline: $\sim$654, no-baseline: $\sim$571), but they exhibit instability. The baseline method shows particularly severe degradation later in training.

Results demonstrate the advantage of PPO over vanilla policy gradient: the clipped objective prevents large policy updates that can destabilize learning.
...
\newpage
\section{Distributions induced by a policy (14 pts)}
...
The probability of sampling a trajectory $\tau$ from running policy $\pi$ in MDP $\mathcal{M}$ is:

\[
\rho^\pi(\tau) = \prod_{t=0}^{\infty} \pi(a_t \mid s_t) P(s_{t+1} \mid s_t, a_t)
\]
...
\item [(b) (1 pt)] (\textbf{written}) Let $p^\pi(s_t = s)$ be the probability of being in state $s$ at timestep $t$ while following policy $\pi$. Write an expression for $p^\pi(s_t = s)$ using $\rho^\pi$.

\begin{tcolorbox}[breakable]
%%%%% Start of 3(b) %%%%%
Answer:

Probability $p^\pi(s_t = s)$ can be expressed by marginalizing over all trajectories:

\[
p^\pi(s_t = s) = \sum_{\tau: s_t = s} \rho^\pi(\tau)
\]

where the sum is over all trajectories $\tau$ that have $s_t = s$.
...
The value function of a policy $\pi$ can be expressed using this distribution $d^{\pi}(s,a) = d^{\pi}(s) \pi(a \mid s)$ over states and actions, which will shortly be quite useful.
 ...
 Consider an arbitrary function $f: \mathcal{S} \times \mathcal{A} \rightarrow \mathbb{R}$. Prove the following identity: $$\mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \gamma^t f(s_t,a_t)\right] = \frac{1}{(1-\gamma)} \mathbb{E}_{s \sim d^\pi}\left[\mathbb{E}_{a \sim \pi(s)}\left[f(s,a)\right]\right] .$$ 
	\emph{Hint: You may find it helpful to first consider how things work out if $f(s,a) = 1$ for a particular state and action tuple $(s, a)$, and $f=0$ anywhere else. This would represent the discounted expected state-and-action visitation count under a policy $\pi$.}
\end{enumerate}	
\begin{tcolorbox}[breakable]
%%%%% Start of 3(c) %%%%%
Answer:
\begin{align*}
\mathbb{E}_{\tau\sim\rho^\pi}\Big[\sum_{t=0}^{\infty}\gamma^t f(s_t,a_t)\Big]
&=\sum_{t=0}^{\infty}\gamma^t\,\mathbb{E}_{\tau\sim\rho^\pi}\big[f(s_t,a_t)\big]\\
&=\sum_{t=0}^{\infty}\gamma^t\sum_s p^\pi(s_t=s)\sum_a \pi(a\mid s)f(s,a)\\
&=\sum_s\Big(\sum_{t=0}^{\infty}\gamma^t p^\pi(s_t=s)\Big)\sum_a \pi(a\mid s)f(s,a)
\end{align*}
...
\begin{align*}
\mathbb{E}_{\tau\sim\rho^\pi}\Big[\sum_{t=0}^{\infty}\gamma^t f(s_t,a_t)\Big]
&= \frac{1}{1-\gamma}\sum_s d^\pi(s)\sum_a \pi(a\mid s)f(s,a) \\
&= \frac{1}{1-\gamma} \mathbb{E}_{s \sim d^\pi}\Big[\mathbb{E}_{a \sim \pi(s)}[f(s,a)]\Big]
\end{align*}
...
\clearpage
 ...
 \begin{enumerate}
    ...
    \emph{Hint 1: Try adding and subtracting a term that will let you bring $A^{\pi'}(s,a)$ into the equation. What happens when adding and subtracting $\sum\limits_{t=0}^\infty \gamma^{t+1} V^{\pi'}(s_{t+1})$ on the LHS?} \\
    ...
    \emph{Hint 3: It may be useful to define $\tau_t = (s_0, a_0, s_1, a_1, .., s_t, a_t)$ as the partial trajectory until timestep $t$. }\\
     \emph{Hint 4: Result from (c) may be useful.}
\begin{tcolorbox}[breakable]
%%%%% Start of 3(d) %%%%%
Answer:
...
\[
Q^{\pi'}(s,a) =
R(s,a) + \gamma\,\mathbb{E}_{s'\sim P(\cdot\mid s,a)}\left[V^{\pi'}(s')\right]
\]
Therefore the advantage can be written as
\begin{align*}
A^{\pi'}(s,a)
&=Q^{\pi'}(s,a)-V^{\pi'}(s)\\
&=R(s,a) +\gamma\,\mathbb{E}_{s'\sim P(\cdot\mid s,a)}\left[V^{\pi'}(s')\right] -V^{\pi'}(s)
\end{align*}

By definition,
\[
V^\pi(s_0) =
\mathbb{E}_{\tau\sim \rho^\pi}\left[\sum_{t=0}^{\infty}\gamma^t R(s_t,a_t)\right]
\]
Hence,
\begin{align*}
V^\pi(s_0)-V^{\pi'}(s_0) &=
...
Then, add and subtract the quantity
$\mathbb{E}_{\tau\sim\rho^\pi}\left[\sum_{t=0}^{\infty}\gamma^{t+1}V^{\pi'}(s_{t+1})\right]$:
\begin{align*}
V^\pi(s_0)-V^{\pi'}(s_0)
&=
\mathbb{E}_{\tau\sim \rho^\pi}\left[\sum_{t=0}^{\infty}\gamma^t R(s_t,a_t)\right]
+ \mathbb{E}_{\tau\sim\rho^\pi}\left[\sum_{t=0}^{\infty}\gamma^{t+1}V^{\pi'}(s_{t+1})\right]\\
&\quad-\Bigg( \mathbb{E}_{\tau\sim\rho^\pi}\left[\sum_{t=0}^{\infty}\gamma^{t+1}V^{\pi'}(s_{t+1})\right]
+ V^{\pi'}(s_0) \Bigg)
\end{align*}

Simplify terms in last parenthesis,
\[
\sum_{t=0}^{\infty}\gamma^{t+1}V^{\pi'}(s_{t+1}) =
\sum_{k=1}^{\infty}\gamma^{k}V^{\pi'}(s_k)
\]
Therefore,
\begin{align*}
\mathbb{E}_{\tau\sim\rho^\pi}\left[\sum_{t=0}^{\infty}\gamma^{t+1}V^{\pi'}(s_{t+1})\right] +V^{\pi'}(s_0)
&= \mathbb{E}_{\tau\sim\rho^\pi}\left[V^{\pi'}(s_0)+\sum_{k=1}^{\infty}\gamma^{k}V^{\pi'}(s_k)\right]\\
&= \mathbb{E}_{\tau\sim\rho^\pi}\left[\sum_{t=0}^{\infty}\gamma^{t}V^{\pi'}(s_t)\right]
\end{align*}

Plugging this back yields
\begin{align*}
V^\pi(s_0)-V^{\pi'}(s_0)
&= \mathbb{E}_{\tau\sim \rho^\pi}\Bigg[\sum_{t=0}^{\infty}\gamma^t\Big(R(s_t,a_t)
+\gamma V^{\pi'}(s_{t+1})-V^{\pi'}(s_t)\Big)\Bigg]
\end{align*}

For a fixed $t$,
\[
X_t = R(s_t,a_t)+\gamma V^{\pi'}(s_{t+1})-V^{\pi'}(s_t)
\]
Applying the tower property of expectation,
\[
\mathbb{E}_{\tau\sim\rho^\pi}[X_t]=
\mathbb{E}_{\tau\sim\rho^\pi}\Big[\mathbb{E}\left[X_t \mid s_t,a_t\right]\Big]
\]

So
\begin{align*}
\mathbb{E}\left[
R(s_t,a_t)+\gamma V^{\pi'}(s_{t+1})\mid s_t,a_t\right]
&=R(s_t,a_t)+\gamma\,\mathbb{E}_{s'\simP(\cdot\mid s_t,a_t)}[V^{\pi'}(s')]\\
&=Q^{\pi'}(s_t,a_t)
\end{align*}
Therefore,
\[
\mathbb{E}\left[X_t \mid s_t,a_t\right]=
Q^{\pi'}(s_t,a_t)-V^{\pi'}(s_t)=
A^{\pi'}(s_t,a_t)
\]

Substituting back into the discounted sum,
\[
V^\pi(s_0)-V^{\pi'}(s_0)=
\mathbb{E}_{\tau\sim \rho^\pi}\left[\sum_{t=0}^{\infty}\gamma^t A^{\pi'}(s_t,a_t)\right]
\]

From part (c), setting $f(s,a)=A^{\pi'}(s,a)$ gives
\[
V^\pi(s_0)-V^{\pi'}(s_0)=
\frac{1}{1-\gamma}\mathbb{E}_{s\sim d^\pi}\left[\mathbb{E}_{a\sim\pi(s)}\big[A^{\pi'}(s,a)\big]\right]
\]
...
\textbf{Respect for Persons:} Researchers outh to obtain informed consent from students and clearly state that the study involves an AI chatbot that may be less effective than human assistance, with the option to request human CA help at any time without penalty. This preserves participants’ autonomy.

\textbf{Beneficence:} Researchers ought to monitor student performance and intervene with human support when needed, and ensure access to supplementary human CA review before assessments so participation does not harm learning outcomes.
...
At universities, research experiments that involve human subjects are subject by federal law to Institutional Review Board (IRB) approval.  The purpose of IRB is to protect human subjects of research: to “assure, both in advance and by periodic review, that appropriate steps are taken to protect the rights and welfare of humans participating as subjects in the research” (\href{https://www.fda.gov/about-fda/center-drug-evaluation-and-research-cder/institutional-review-boards-irbs-and-protection-human-subjects-clinical-trials}{link to reference}) The IRB process was established in response to abuses of human subjects in the name of medical research performed during WWII (\href{https://www.hhs.gov/ohrp/regulations-and-policy/belmont-report/read-the-belmont-report/index.html#xethical}{link to reference}) The IRB is primarily intended to address the responsibilities of the researcher towards the subjects. Familiarize yourself with Stanford’s IRB Research Compliance process by clicking the \href{https://researchcompliance.stanford.edu/panels/hs/forms/for-researchers#need}{link to reference}.
...
I would need to submit a research protocol through Stanford's IRB (Institutional Review Board) system. According to the website, I would:

\begin{enumerate}
    \item Create an account and submit the protocol through Stanford's eProtocol system
    \item Complete the appropriate IRB application form for human subjects research
    \item Provide detailed information about the study
    \item Wait for IRB review and approval before beginning any data collection or participant recruitment
\end{enumerate}
