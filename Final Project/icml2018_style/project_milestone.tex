...
\subsection{Vehicle model and spatial direct collocation}

We use a single-track (bicycle) dynamics model with nonlinear tire forces and load transfer following Aggarwal \& Gerdes~\cite{aggarwal2025friction}. The trajectory is parameterized by arc length $s$ to align constraints with track geometry. Let $k=0,\dots,N$ index spatial nodes with spacing $\Delta s$, decision variables $\mathbf{X}=[\mathbf{x}_0,\dots,\mathbf{x}_N]$ and $\mathbf{U}=[\mathbf{u}_0,\dots,\mathbf{u}_N]$, and state/control:
\begin{equation}
\mathbf{x} = [u_x, u_y, r, \Delta F_{z,\text{long}}, \Delta F_{z,\text{lat}}, t, e, \Delta\psi]^\top,\quad
\mathbf{u} = [\delta, F_x]^\top.
\end{equation}
The objective is minimum lap time with a lightweight ``anti-chatter'' regularizer:
\begin{equation}
\min_{\mathbf{X},\mathbf{U}} \; J
= t_N
+ \lambda_u \sum_{k=0}^{N-1}\|\mathbf{u}_{k+1}-\mathbf{u}_k\|_2^2.
\end{equation}
Dynamics are enforced by trapezoidal collocation in the spatial domain:
\begin{equation}
...
\end{align}
For full-lap solutions we impose periodic closure $\mathbf{x}_0=\mathbf{x}_N$ and $\mathbf{u}_0=\mathbf{u}_N$ for all states/controls except time (with $t_0=0$ and $t_N$ free).
...
\fbox{\parbox{\columnwidth}{\vspace{0.5cm}\centering Place \texttt{results/trajectory\_optimization/ipopt\_single\_stage\_trajectory.png} here.\vspace{0.5cm}}}
}
