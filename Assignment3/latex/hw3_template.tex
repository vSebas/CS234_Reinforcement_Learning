...
\begin{enumerate}
    ...
    \answer{
    %%%%% Start of 1(a) %%%%%
    Reward engineering is hard because:
    \begin{itemize}
        \item It requires precisely translating complex human goals into a mathematical function, which is inherently ambiguous.
        \item The reward function must capture all aspects of desired behavior, but humans often have implicit preferences that are difficult to formalize.
        \item Small changes in the reward can lead to drastically different learned behaviors (reward hacking).
    \end{itemize}

    \textbf{Potential risks} of incorrect reward functions include:
    \begin{itemize}
        \item \textbf{Reward hacking}: The agent finds unintended ways to maximize reward that don't align with the true objective.
        \item \textbf{Side effects}: The agent may cause harm to achieve its goal because the reward doesn't penalize such behaviors.
        \item \textbf{Distributional shift}: The reward may work well in training but fail in deployment scenarios.
    \end{itemize}

    \textbf{Example}: Consider a cleaning robot with reward function $r = \text{(amount of dirt cleaned)}$. This appears adequate, but the robot might learn to:
    \begin{itemize}
        \item Spread dirt around first, then clean it (creating more opportunities for reward)
        \item Damage furniture to access hard-to-reach dirt
        \item Operate continuously without regard for energy consumption or noise
    \end{itemize}
    %%%%% End of 1(a) %%%%%
    }
    ...
    \begin{itemize}
        \item \textbf{Healthy reward} ($r_{\text{healthy}} = 1$): A constant reward given at each timestep when the hopper is in a ``healthy'' state. This encourages the agent to stay upright and balanced, preventing it from falling.

        \item \textbf{Forward reward} ($r_{\text{forward}} = v_x \cdot \text{forward\_reward\_weight}$): Proportional to the hopper's forward velocity. This directly incentivizes the agent to move forward as fast as possible, which is the primary objective.

        \item \textbf{Control cost} ($r_{\text{ctrl}} = \text{ctrl\_cost\_weight} \cdot \|a\|_2^2$): A penalty proportional to the squared magnitude of the actions. This discourages the agent from using unnecessarily large control inputs, promoting energy-efficient and smooth movements.
    \end{itemize}
    %%%%% End of 1(b) %%%%%
    }
    ...
    \answer{
    %%%%% Start of 1(c) %%%%%
    \textbf{Healthy states} are defined by the following conditions:
    \begin{itemize}
        \item The hopper's height (z-coordinate of the torso) is within $[0.7, +\infty)$ meters
        \item The torso angle is within $[-0.2, 0.2]$ radians
        \item All state values are finite (no NaN or Inf)
    \end{itemize}

    These constraints ensure the hopper remains upright and hasn't fallen over.

    \textbf{Advantage of early termination}: It significantly speeds up training by not wasting time simulating states where the robot has already fallen and cannot recover. This allows the agent to focus on learning productive behaviors rather than exploring unrecoverable failure states.

    \textbf{Disadvantage of early termination}: It can bias the learned policy by preventing the agent from learning how to recover from near-failure states. The agent may become overly conservative or brittle, never learning to operate near the boundaries of the healthy region where potentially efficient but risky behaviors exist.
    %%%%% End of 1(c) %%%%%
    }
...
\end{tcolorbox}
    ...
    \begin{itemize}
        \item \textbf{With early termination}: Training converges faster because episodes terminate quickly when the agent falls, allowing more episodes to be run in the same wall time. The agent learns to maintain balance earlier.
        \item \textbf{Without early termination}: Training is slower per epoch because episodes run to completion even after falling. However, the agent may learn more robust behaviors since it experiences the full state space.
    \end{itemize}

    \textbf{Standard error}: The standard error across seeds is moderate, indicating some variability in training outcomes. This is expected in RL due to exploration randomness and environment stochasticity.

    \textbf{Better estimate}: To obtain a better estimate of the average return, we could:
    \begin{itemize}
        \item Use more random seeds (e.g., 10+ seeds) to reduce variance in the estimate
        \item Run more evaluation episodes per checkpoint
        \item Use a larger evaluation seed set that differs from training seeds
        \item Apply bootstrapping or cross-validation techniques
    \end{itemize}
    %%%%% End of 1(d) %%%%%
    }
    ...
    \answer{
    %%%%% Start of 1(e) %%%%%
    Yes, the agent successfully learns to hop forward. The hopper maintains balance and achieves forward locomotion throughout the episode.

    The behavior is somewhat surprising in that the learned hopping gait may not look like a natural or intuitive hopping motion. The agent discovers an efficient but unusual gait that maximizes the reward function---demonstrating how RL agents can find unexpected solutions that technically satisfy the objective without matching human intuitions about ``correct'' hopping.
    %%%%% End of 1(e) %%%%%
    }
    ...
    \answer{
    %%%%% Start of 1(f) %%%%%
    Comparing two policies (e.g., one trained with early termination and one without, or two different seeds):

    The two rollouts show different hopping styles/gaits. Key differences may include:
    \begin{itemize}
        \item \textbf{Speed}: One policy may achieve faster forward velocity
        \item \textbf{Stability}: One may appear more balanced or less wobbly
        \item \textbf{Efficiency}: One may use smoother, more economical movements
        \item \textbf{Risk tolerance}: The policy trained without early termination might take riskier positions
    \end{itemize}

    Personal preference would depend on the specific realizations. A policy that achieves high reward while appearing stable and natural would be preferred. However, both demonstrate the core capability of forward locomotion, highlighting that multiple solutions can achieve similar objective values while exhibiting qualitatively different behaviors.
    %%%%% End of 1(f) %%%%%
    }
