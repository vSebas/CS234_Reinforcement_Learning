...
After viewing 5 different samples from the long preferences dataset, I can say that I agree with 4 out of the 5 labels. For sample with $\text{IDX}=1$, I preferred the one labeled as "Not Preferred". 

I guess my estimated agreement rate is approximately 80\%. I would trust in the reward function on this data, but with caveats. This high agreement suggests that the labels capture reasonable human intuitions about good hopping behavior, tough some edge cases may still seem ambiguous.
...
The behavior of the RLHF-trained agent demonstrates smoother and more stable hopping, better than results from problem 1 (both types). In particular, \textit{Hopper-v4-rlhf-seed=1} maintains a much better balance, though at the cost of reduced forward velocity.

Compared againts the dataset demonstrations, I can say that they seem like the preferred sequence, though these new results do not freeze, and are more complete.
...
\textbf{Results}: DPO significantly outperforms SFT. SFT achieves final returns of $\sim$440--480, while DPO achieves $\sim$930--960 (roughly 2x better). For comparison, RLHF achieved $\sim$930--1495.

\textbf{Comparison with RLHF}: DPO reaches similar performance to RLHF on some seeds, but RLHF has higher potential (seed 1 reached 1495). This is because RLHF uses online RL to actively explore, while DPO is limited to offline data.

\textbf{Pros and cons}:
\begin{itemize}
    \item \textbf{RLHF}: Can achieve higher returns through online exploration, but requires training a separate reward model and is more computationally expensive.
    \item \textbf{DPO}: Simpler (no reward model needed), more stable training, but limited to behaviors in the dataset and cannot discover new strategies.
    \item \textbf{SFT}: Simplest approach (just imitation), but ignores preference information and performs worst.
\end{itemize}
%%%%% End of 3(d) %%%%%
}
...
\begin{enumerate}
    ...
    The event tjat some arm has a large deviation, ''$\exists a \in \mathcal{A}$ s.t. $|\widehat{r}_a - \overline{r}_a| > \sqrt{\frac{\log(2/\delta)}{2n_e}}$'', is exactly $\bigcup_{a \in \mathcal{A}} B_a$.
    ...
    Therefore:
    \[\Pr\left(\exists a \in \mathcal{A} \text{ s.t. } |\widehat{r}_a - \overline{r}_a| > \sqrt{\frac{\log(2/\delta)}{2n_e}}\right) < |\mathcal{A}|\delta\]
    %%%%% End of 4(a) %%%%%
    }
    ...
    To guarantee $\epsilon$-optimality, we need each estimate to be within $\frac{\epsilon}{2}$ of its true mean.

    Suppose $|\widehat{r}_a - \overline{r}_a| \leq \frac{\epsilon}{2}$ for all $a$ (good event). Then:
    \begin{align*}
    ...
    \overline{r}_{a^\dagger} &\geq \overline{r}_{a^\star} - \epsilon
    \end{align*}

    So $\overline r_{a^\dagger}$ is $\epsilon$-optimal whenever all arms are estimated within $\epsilon/2$
...
\begin{enumerate}
    ...
    \textbf{Stated preferences}: user profiles (topics users say they are interested in and preferred format)
    \textbf{Revealed preferences}: interaction data and engagement metrics
    %%%%% End of 5(a) %%%%%
    }
    ...
    \[r = c_1 \cdot \text{engagement} + c_2 \cdot \text{retention} + c_3 \cdot \text{revenue}\]

    Could also include info about return visits, time spent on app, user clicks on articles, etc.
    %%%%% End of 5(b) %%%%%
    }
    ...
    Prioritizing revealed preferences risks exploiting psychological vulnerabilities like doom-scrolling or outrage addiction, and can create filter bubbles where users only see reinforcing content. On the other hand, stated preferences respect user autonomy but may be aspirational rather than realistic (as in users don't always know what they truly want).
    ...
    We could occasionally show content outside the user's known preferences and observe whether they engage with it (if they do, it signals their preferences may be evolving). A simple approach is $\epsilon$-greedy exploration: with small probability (e.g., 5--10\%), recommend content from topics the user hasn't explicitly preferred, then track engagement. If the user interacts positively, update their preference model accordingly.
    %%%%% End of 5(d) %%%%%
    }
