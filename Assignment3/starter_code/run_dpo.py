...
class ActionSequenceModel(nn.Module):
    def __init__(
        ...
        1. The action distribution will need a mean and std for each dimension of the action.
        2. For each observation, your network should have output with dimension 2 * self.segment_len * self.action_dim.
        3. Use ReLU activations, and have the last layer be a linear layer.
    ...
    def act(self, obs: np.ndarray) -> np.ndarray:
        ...
        reduce evaluation variance, and potentially better performance
        thanks to less noise.

        If you are interested, you can compare the difference between
        using the mean versus sampling from the distribution.
        Remember to clamp the action values between -1 and 1 if you sample.
        """
        #######################################################
